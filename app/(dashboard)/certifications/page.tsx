...
import { AlertTriangle, CheckCircle, Clock, Info, Search, XCircle } from 'lucide-react';
...
export default function CertificationsPage() {
      ...
      try {
        ...
        const url =
          isSnapshotMode && snapshotDate
            ? `/api/certifications?asOfDate=${encodeURIComponent(snapshotDate)}`
            : '/api/certifications';
  ...
  const filteredCertifications = certifications.filter((c: any) => {
    ...
    const matchesSearch =
      !q || c.type?.toLowerCase().includes(q) || c.issuing_authority?.toLowerCase().includes(q);
        ...
        <div className="flex items-start gap-4">
          <div className="flex-1">
            ...
            {isSnapshotMode && snapshotDate && (
              <p className="mt-2 text-sm text-slate-400">
                Showing results as of{' '}
                <span className="text-white">{new Date(snapshotDate).toLocaleDateString()}</span>
              </p>
            )}
          </div>
          ...
          <div className="shrink-0">
            <button
              ...
              className="px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg hover:bg-slate-800 text-white text-sm font-medium"
            >
                ...
                filteredCertifications.map((cert: any) => (
                  <tr key={cert.id} className="hover:bg-slate-800/50">
                    ...
                    <td className="px-6 py-4">
                      <Link
                        href={`/employees/${cert.employee_id}`}
                    ...
                    <td className="px-6 py-4">
                      {cert.status === 'PASS' ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-3 py-1 bg-amber-900/30 text-amber-300 rounded-full text-xs font-medium">
